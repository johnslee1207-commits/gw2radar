...
def test_kb_semantic_maturity_report_prioritizes_batch_promotion_after_rule_packs() -> None:
    ...
    assert report.recommended_priorities[0].priority_id == "P13"
    assert "KB Batch Validation and Promotion Planner" in markdown
...
def test_kb_semantic_maturity_api_exports_markdown() -> None:
    ...
    assert response.json()["data"]["report"]["recommended_priorities"][0]["priority_id"] == "P13"
