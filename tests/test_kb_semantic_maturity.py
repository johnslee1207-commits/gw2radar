...
def test_kb_semantic_maturity_report_prioritizes_domain_rule_packs() -> None:
    ...
    assert report.recommended_priorities[0].priority_id == "P12"
    assert "Reviewed Returner/Build/Market Rule Packs" in markdown
...
def test_kb_semantic_maturity_api_exports_markdown() -> None:
    ...
    assert response.json()["data"]["report"]["recommended_priorities"][0]["priority_id"] == "P12"
